...
- **مقاعد القرّاء**: منح مقعد قراءة فقط بمدة محددة — للقرّاء الضيوف أو الطلاب أو تسليم العملاء. ينتهي تلقائياً عند انتهاء صلاحيته (فشل مغلق عند حل البيانات، دون cron). أدرها من صفحة الأعضاء، أو افتحها بالبريد عبر نقطة النهاية `PUT /api/app/admin/team/reader` باستخدام رمز وصول شخصي (انظر `docs/team-mode.md`).
