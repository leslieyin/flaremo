...
- `AIService/Transcribe`、Instance email test、OAuth2/IdentityProvider CRUD、linked identity CRUD 当前明确未实现或返回 `501`；User create/delete 已由 owner 会话授权实现（`CreateUser`/`DeleteUser`），但完整上游 webhook 事件/egress 语义、完整通知 filter 和多用户 ACL 仍未完成。
